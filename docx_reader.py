...
class DocxReader:
    ...
    def parse_docx_files(self):
        for document in os.listdir(self.docs_route):
            dict = self.parse_docx(docx.Document(f"{self.docs_route}/{document}"))
            self.dictionaries.append(dict)
        return self.dictionaries

    def parse_docx(self, document):
        paragraphs = [p.text for p in document.paragraphs]
        dict = self.make_dictionary(paragraphs)
        img_routes = self.extract_images(document, dict['dni'])
        if img_routes:
            dict[IMG_PROPERTY] = img_routes[0]
        return dict
...
if __name__ == '__main__':
    dx_reader = DocxReader(DOCS_FOLDER, IMAGES_FOLDER)
